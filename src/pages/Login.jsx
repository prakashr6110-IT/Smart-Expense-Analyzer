...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    setLoading(true);
    try { await login(email, password); navigate('/dashboard'); } catch (err) { setError(err.message || 'Failed to login'); } finally { setLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-fintech-bg flex items-center justify-center px-4 animate-fade-in">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 -left-20 w-72 h-72 bg-accent-primary/10 rounded-full blur-[120px]" />
        <div className="absolute bottom-1/4 -right-20 w-72 h-72 bg-accent-insights/10 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="card w-full max-w-md animate-scale-in relative z-10">
        ...
        <h2 className="text-2xl font-heading font-bold text-slate-800 dark:text-txt-primary mb-6">Login to your account</h2>
        ...
        <p className="text-center text-slate-400 dark:text-txt-muted mt-6">
          ...
          <Link to="/register" className="text-accent-primary hover:text-accent-primary/80 font-medium transition-colors">Sign up</Link>
        </p>
      </div>
    </div>
  );
